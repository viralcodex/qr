...
public class QREncoder {
    ...
    private static String getPaddedEncodedString(int version, ErrorCorrection errorCorrectionLevel,
            String encodedString, String indicators) {
        int totalBitsAllowed = Constants.DATA_CODEWORDS[version - 1][errorCorrectionLevel.ordinal()] * 8; // bits =
                                                                                                          // bytes * 8
        ...
        sb.append(indicators).append(encodedString).append("0".repeat(Math.min(difference, 4))); // only upto 4 0s to
                                                                                                 // add
    ...
    private static String getErrorCorrectionCodedString(int version, ErrorCorrection errorCorrectionLevel,
            String paddedEncodedString) {
        ...
        for (int i = 0; i < dataBlocks.length; i++) {
            errorCorrectionCodewords[i] = getErrorCorrectionCodewords(dataBlocks[i], generatorPolynomial,
                    ecBlockInfo[0]);
        }
         ...
         * so we loop through the data blocks and take the codewords at the same index
         * (if it exists) and append to the final string.
         * We do this for all indices up to the maximum number of data codewords in any
         * block.
         * This way we are interleaving the codewords from each block together.
         * After interleaving all the data codewords, we then interleave the error
         * correction codewords in the same way.
         */
        for (int col = 0; col < maxDataCodewords; col++) {
            for (int block = 0; block < dataBlocks.length; block++) {
                if (col < dataBlocks[block].length) {
                    sb.append(String.format("%8s", Integer.toBinaryString(dataBlocks[block][col] & 0xFF)).replace(' ',
                            '0'));
                }
            }
        }
        ...
        for (int col = 0; col < ecBlockInfo[0]; col++) {
            for (int block = 0; block < errorCorrectionCodewords.length; block++) {
                if (col < errorCorrectionCodewords[block].length) {
                    sb.append(String.format("%8s", Integer.toBinaryString(errorCorrectionCodewords[block][col] & 0xFF))
                            .replace(' ', '0'));
                }
            }
        }
    ...
    private static int[][] getDataBlocks(String paddedEncodedString, int[] ecBlockInfo) {
        ...
        for (int i = 0; i < group2Blocks; i++) {
            ...
            for (int j = 0; j < group2DataCodewords; j++) {
                dataBlocks[group1Blocks + i][j] = Integer
                        .parseInt(paddedEncodedString.substring(bitOffset, bitOffset + 8), 2);
                bitOffset += 8;
            }
        }
    ...
    private static int[] getGeneratorPolynomial(int degree) {
        ...
        for (int i = 0; i < degree; i++) {
            ...
            generatorPolynomial = polynomialMultiply(generatorPolynomial, term); // multiply the current generator
                                                                                 // polynomial by the new term to get
                                                                                 // the updated generator polynomial
        }
     ...
     * 3. divide that message polynomial by the generator polynomial using
     * polynomial long division in GF(256)
     * 4. the remainder from that division is the error correction codewords for
     * that block
     * 5. repeat for each block
     */
    private static int[] getErrorCorrectionCodewords(int[] dataBlocks, int[] generatorPolynomial,
            int ecCodewordsPerBlock) {
        int dataLength = dataBlocks.length;
        int[] workingPolynomial = new int[dataLength + ecCodewordsPerBlock]; // the message polynomial [data block
                                                                             // codewords + error correction codewords]
        ...
        for (int i = 0; i < dataLength; i++) {
            ...
            if (leadTerm != 0) {
                // multiply the generator polynomial by the lead term and subtract (XOR) from
                // the working polynomial
                for (int j = 0; j < generatorPolynomial.length; j++) {
                    int product = GaloisField.gfMultiply(leadTerm, generatorPolynomial[j]);
                    workingPolynomial[i + j] = GaloisField.gfAdd(workingPolynomial[i + j], product); // XOR for
                                                                                                     // subtraction in
                                                                                                     // GF(256)
                }
            }
        ...
        // the last ecCodewordsPerBlock terms of the working polynomial are the error
        // correction codewords after the division process
